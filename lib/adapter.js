    ...
    describe: function(connectionName, table, cb) {
      spawnConnection(connectionName, function __DESCRIBE__(client, cb) {
        ...
        var query = "SELECT x.nspname || '.' || x.relname as \"table\", x.attnum as \"#\", x.attname as \"column\", x.\"type\"," +
          ...
          "r.consrc, fn.nspname || '.' || f.relname as \"f key\", d.adsrc as \"defalut\" FROM (" +
    ...
    create: function(connectionName, table, data, cb) {
      spawnConnection(connectionName, function __CREATE__(client, cb) {
        ...
        client.query(query.query, query.values, function __CREATE__(err, result) {
          ...
          var getDateIndexes = function(query) {
            var upIdx, crIdx;
            var q_arr = query.split(/\(|\)/);
            for (var q in q_arr) {
              var q_row = q_arr[q];
              if (q_row.indexOf("updatedAt") !== -1 && q_row.indexOf("createdAt") !== -1) {
                var q_in = q_row.split(',');
                var i;
                for (i=0;i< q_in.length;i++) {
                  var q_i = q_in[i];
                  if (q_i.indexOf("updatedAt") !== -1) {
                    upIdx = i;
                  } else if (q_i.indexOf("createdAt") !== -1) {
                    crIdx = i;
                  }
                  if (upIdx && crIdx) break;
                }
              }
              if (upIdx && crIdx) break;
            }
          ...
          // Set Sequence value to defined value if needed, or return updated record assuming a created and updated
          // date are appended
          if(incrementSequences.length === 0) {
            var dateIdx = getDateIndexes(query.query);
            data["createdAt"] = query.values[dateIdx[0]];
            data["updatedAt"] = query.values[dateIdx[1]];

            // Build a query for the specific query strategy
            var _query;
            try {
              var options = { "where": data };
