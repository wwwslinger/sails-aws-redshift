...
utils.buildSchema = function(obj) {
  var columns = _.map(obj, function (attribute, name) {
    ...
    var type = utils.sqlTypeCast(attribute.autoIncrement ? 'BIGINT' : attribute.type);
    var nullable = attribute.notNull && 'NOT NULL';
    var unique = attribute.unique && 'UNIQUE';
    var identity = attribute.autoIncrement && 'IDENTITY(0, 1)'

    return _.compact([ '"' + name + '"', type, identity, nullable, unique ]).join(' ');
  }).join(',');
